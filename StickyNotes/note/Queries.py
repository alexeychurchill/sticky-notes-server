...
INSERT INTO note_text(note_id, text) VALUES (@new_note_id, \"\");
COMMIT;
...
SELECT
    ...
    UNIX_TIMESTAMP(change_date),
    owner_id 
FROM note WHERE id={id} AND owner_id={owner_id} LIMIT 1
"""
...
SELECT
    ...
    UNIX_TIMESTAMP(change_date),
    owner_id 
FROM note WHERE owner_id={owner_id} ORDER BY id DESC LIMIT {offset},{count}
"""

QUERY_GET_NOTES_LIST = """
SELECT
    id,
    title,
    subject,
    ...
    UNIX_TIMESTAMP(change_date),
    owner_id 
FROM note_entry WHERE owner_id={owner_id} ORDER BY id DESC LIMIT {offset},{count}
"""
